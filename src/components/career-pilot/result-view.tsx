    ...
    <Card className="w-full max-w-2xl mx-auto border-primary/20 shadow-primary/5 shadow-lg">
      <CardHeader>
        <div className="flex justify-between items-start">
          ...
          <div className={`flex items-center gap-2 p-2 rounded-md font-semibold ${isApproved ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
      ...
      <CardContent className="space-y-6">
        <div className="text-center py-6 bg-muted/50 rounded-lg">
          ...
          <p className={`font-bold text-6xl font-headline ${isApproved ? 'text-green-400' : 'text-yellow-400'}`}>{result.matchScore}%</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2 text-yellow-400"><XCircle size={18} /> Missing Skills</h3>
            <div className="flex flex-wrap gap-2">
              {result.missingSkills.length > 0 ? result.missingSkills.map(skill => (
                <Badge key={skill} variant="outline" className="text-yellow-300 border-yellow-500/30">{skill}</Badge>
